...
BASE_MAPPING = [
    CharConfigField(
        "global.admin_email",
        "ADMIN_EMAIL",
        help_str=_("Logged errors are sent to this email address."),
    ...
    CharConfigField(
        ...
        "SECRET_KEY",
        help_str=_(
            "Secret key to provide cryptographic signing, and should be set to a unique, unpredictable value."
        ),
        env_name="SECRET_KEY",
    ),
    DirectoryPathConfigField(
    ...
    DirectoryPathConfigField(
        ...
        "MEDIA_ROOT",
        help_str=_("Where uploaded media files will be stored."),
    ...
    CharConfigField(
        ...
        help_str=_(
            "S3 storage regions, when S3 storage is used for storing uplaoded media."
        ),
        env_name="S3_REGION",
    ),
    DirectoryPathConfigField(
        ...
        help_str=_("Where temporary data will be stored."),
        env_name="TEMP_DIRECTORY",
    ),
    CharConfigField(
        "global.language_code",
        "LANGUAGE_CODE",
        help_str=_("Default to fr_FR"),
        env_name="LANGUAGE_CODE",
    ),
    CharConfigField(
        "global.listen_address",
        "LISTEN_ADDRESS",
        help_str=_("Address listen by your web server (like 127.0.0.1:8000 or :8000)."),
    ...
    CharConfigField(
        None,
        "HEROKU_APP_NAME",
        help_str=_("Heroku's application name, when deploying your on Heroku."),
        env_name="HEROKU_APP_NAME",
    ),
    CharConfigField(
        "global.time_zone",
        "TIME_ZONE",
        help_str=_("Default to Europe/Paris"),
        env_name="TIME_ZONE",
    ),
    CharConfigField(
        None,
    ...
    CharConfigField("email.host", "EMAIL_HOST", help_str=_("SMTP server")),
    CharConfigField(
        "email.password", "EMAIL_HOST_PASSWORD", help_str=_("SMTP password")
    ),
    IntegerConfigField(
        "email.port", "EMAIL_PORT", help_str=_("SMTP port (often 25, 465 or 587)")
    ),
    CharConfigField("email.user", "EMAIL_HOST_USER", help_str=_("SMTP user")),
    CharConfigField(
        "email.from",
        "DEFAULT_FROM_EMAIL",
        help_str=_("Displayed sender email"),
        env_name="EMAIL_FROM",
    ),
    BooleanConfigField(
        "email.use_tls",
        "EMAIL_USE_TLS",
        help_str=_('"true" if your SMTP uses STARTTLS ' "(often on port 587)"),
    ),
    BooleanConfigField(
        "email.use_ssl",
        "EMAIL_USE_SSL",
        help_str=_('"true" if your SMTP uses SSL (often on port 465)'),
    ),
]
CELERY_MAPPING = [
    IntegerConfigField(
    ...
    CharConfigField("celery.host", "CELERY_HOST", help_str=_("Redis Celery DB host")),
    CharConfigField(
        "celery.password",
        "CELERY_PASSWORD",
        help_str=_("Redis Celery DB password (if required)"),
    ),
    IntegerConfigField(
        "celery.port", "CELERY_PORT", help_str=_("Redis Celery DB port")
    ),
    IntegerConfigField(
        "celery.processes", "CELERY_PROCESSES", help_str=_("number of Celery processes")
    ),
]
DATABASE_MAPPING = [
    CharConfigField(
        ...
        "DATABASE_URL",
        help_str=_(
            "URL of the database (e.g. (e.g. (postgres|mysql)://username:password@127.0.0.1:5432/database"
    ...
    CharConfigField(
        "database.db",
        "DATABASE_NAME",
        help_str=_("Database name (or path of the sqlite3 database)"),
        env_name=None,
    ),
    CharConfigField(
        "database.engine",
        "DATABASE_ENGINE",
        help_str=_(
            'Database engine ("mysql", "postgresql", "sqlite3", "oracle", or the dotted name of '
            "the Django backend)"
        ),
        env_name=None,
    ),
    CharConfigField(
        "database.host", "DATABASE_HOST", help_str=_("Database host"), env_name=None
    ),
    CharConfigField(
        "database.password",
        "DATABASE_PASSWORD",
    ...
    IntegerConfigField(
        "database.port", "DATABASE_PORT", help_str=_("Database port"), env_name=None
    ),
    CharConfigField(
        "database.user", "DATABASE_USER", help_str=_("Database user"), env_name=None
    ),
]
DJANGO_AUTH_MAPPING = [
    BooleanConfigField(
        ...
        help_str=_('Set to "false" to deactivate local database of users.'),
    ),
    BooleanConfigField(
        "auth.create_users",
        "DF_ALLOW_USER_CREATION",
        help_str=_(
            'Set to "false" if users cannot create their account themselvers, or '
    ...
    IntegerConfigField(
        ...
        "SESSION_COOKIE_AGE",
        help_str=_(
            "Duration of the connection sessions "
...
LDAP_AUTH_MAPPING = [
    CharConfigField(
        ...
        help_str=_(
            'URL of your LDAP server, like "ldap://ldap.example.com". '
    ...
    CharConfigField(
        "auth.ldap_bind_dn",
        "AUTH_LDAP_BIND_DN",
        help_str=_("Bind dn for LDAP authentication"),
    ),
    CharConfigField(
        "auth.ldap_bind_password",
        "AUTH_LDAP_BIND_PASSWORD",
        help_str=_("Bind password for LDAP authentication"),
    ),
    CharConfigField(
        "auth.ldap_user_search_base",
        "AUTH_LDAP_USER_SEARCH_BASE",
        help_str=_(
            "Search base for LDAP authentication by direct after an search, "
    ...
    CharConfigField(
        ...
        "AUTH_LDAP_FILTER",
        help_str=_(
            'Filter for LDAP authentication, like "(uid=%%(user)s)" (the default),'
    ...
    CharConfigField(
        ...
        help_str=_(
            "Set it for a direct LDAP bind and to skip the LDAP search, "
    ...
    BooleanConfigField(
        "auth.ldap_start_tls",
        "AUTH_LDAP_START_TLS",
        help_str=_('Set to "true" if you want to use StartTLS.'),
    ),
    CharConfigField(
        "auth.ldap_first_name_attribute",
        "AUTH_LDAP_USER_FIRST_NAME",
        help_str=_('LDAP attribute for the user\'s first name, like "givenName".'),
    ),
    CharConfigField(
        "auth.ldap_last_name_attribute",
        "AUTH_LDAP_USER_LAST_NAME",
        help_str=_('LDAP attribute for the user\'s last name, like "sn".'),
    ),
    CharConfigField(
        "auth.ldap_email_attribute",
        "AUTH_LDAP_USER_EMAIL",
        help_str=_('LDAP attribute for the user\'s email, like "email".'),
    ),
    CharConfigField(
    ...
    CharConfigField(
        ...
        help_str=_(
            "Authentication is denied for users belonging to this group. Must be something like "
    ...
    BooleanConfigField(
        ...
        help_str=_("Mirror LDAP groups at each user login"),
    ),
    CharConfigField(
        "auth.ldap_group_search_base",
        "AUTH_LDAP_GROUP_SEARCH_BASE",
        help_str=_('Search base for LDAP groups, like "ou=groups,dc=example,dc=com"'),
    ),
    ChoiceConfigFile(
        ...
        help_str=_("Type of LDAP groups."),
    ),
]
HTTP_AUTH_MAPPING = [
    CharConfigField(
        ...
        help_str=_(
            'Set it if the reverse-proxy authenticates users, a common value is "HTTP_REMOTE_USER". '
...
LOG_MAPPING = [
    ...
    CharConfigField(
        ...
        "SENTRY_DSN",
        help_str=_("sentry DSN (see https://sentry.io/)"),
        env_name="SENTRY_DSN",
    ),
    CharConfigField(
        "global.log_directory",
        "LOG_DIRECTORY",
        help_str=_("Write all local logs to this directory."),
    ...
    BooleanConfigField(
        ...
        "LOG_REMOTE_ACCESS",
        help_str=_("If true, log of HTTP connections are also sent to syslog/logd."),
    ),
    FloatConfigField(
        "global.log_slow_query_duration_in_s",
        "LOG_SLOW_QUERY_DURATION_IN_S",
        help_str=_("Log slow queries that take more than this time (in seconds)."),
...
PAM_AUTH_MAPPING = [
    BooleanConfigField(
        ...
        help_str=_('Set to "true" if you want to activate PAM authentication'),
    ),
]
RADIUS_AUTH_MAPPING = [
    CharConfigField(
    ...
    CharConfigField(
        "auth.radius_secret",
        "RADIUS_SECRET",
        help_str=_("Shared secret if the Radius server"),
    ),
]
REDIS_MAPPING = [
    CharConfigField(
        ...
        "COMMON_REDIS_URL",
        help_str=_(
            "Redis database URL, for all Redis things (e.g. rediss://django:mysecret@redis.example.com:6379/0"
    ...
    CharConfigField(
        "cache.host", "CACHE_HOST", help_str=_("Cache server host (redis or memcache)")
    ),
    CharConfigField(
        "cache.password",
        "CACHE_PASSWORD",
        help_str=_("Cache server password (if required by redis)"),
    ),
    IntegerConfigField(
    ...
    ChoiceConfigFile(
        ...
        help_str=_('Cache storage engine ("locmem", "redis" or "memcache")'),
    ),
    IntegerConfigField(
        "sessions.db",
        "SESSION_REDIS_DB",
    ...
    CharConfigField(
        "sessions.host", "SESSION_REDIS_HOST", help_str=_("Redis sessions DB host")
    ),
    CharConfigField(
        "sessions.password",
        "SESSION_REDIS_PASSWORD",
        help_str=_("Redis sessions DB password (if required)"),
    ),
    IntegerConfigField(
        "sessions.port", "SESSION_REDIS_PORT", help_str=_("Redis sessions DB port")
    ),
]
SENDFILE_MAPPING = [
    BooleanConfigField(
        ...
        "USE_X_SEND_FILE",
        help_str=_(
            '"true" if Apache is used as reverse-proxy with mod_xsendfile.'
    ...
    ConfigField(
        ...
        help_str=_(
            '"true" is nginx is used as reverse-proxy with x-accel-redirect.'
